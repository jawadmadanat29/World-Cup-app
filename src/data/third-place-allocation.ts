// ============================================================================
// FIFA 2026 World Cup — Round of 32 third-placed-team allocation (Annex C).
//
// The eight best third-placed teams are NOT slotted into the Round of 32 in
// any order — FIFA assigns them by a fixed table that depends on WHICH eight
// of the twelve groups produced a qualifying third. This is that table, all
// C(12,8) = 495 combinations, parsed verbatim from the official bracket
// (en.wikipedia.org/wiki/2026_FIFA_World_Cup_knockout_stage). Each row was
// validated to be a permutation respecting every winner slot's allowed groups.
//
// KEY:   the 8 qualifying group letters, uppercased + sorted (e.g. "EFGHIJKL").
// VALUE: the third-placed GROUP that faces each group winner, in the winner
//        order below (THIRD_WINNERS). e.g. value[0] is the group whose third
//        plays the winner of Group A.
// ============================================================================

/** Group winners that face a third-placed team in the Round of 32, in column order. */
...
/**
 * Given the 8 groups whose third-placed teams advanced, return the mapping of
 * each group winner (A,B,D,E,G,I,K,L) to the GROUP whose third-placed team it
 * faces in the Round of 32. Returns null until exactly 8 distinct groups are
 * supplied (i.e. the pick is incomplete) or if the combination is unknown.
 */
export function thirdPlaceAssignment(qualifyingGroups: string[]): Record<string, string> | null {
  const key = [...new Set(qualifyingGroups.map((g) => g.toUpperCase()))].sort().join("");
  if (key.length !== 8) return null;
  const value = ALLOCATION[key];
  if (!value) return null;
  const out: Record<string, string> = {};
  THIRD_WINNERS.forEach((w, i) => { out[w] = value[i]; });
  return out;
}
